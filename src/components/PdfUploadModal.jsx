...
const PdfUploadModal = ({ imageOpen, setImageUploadOpen, rowData }) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (categories.length > 0) {
          handleMainCategorySelect(categories[0]); // auto-select first
        }
      })
      .catch(() => {
  ...
  const handleFilesForSubcategory = (subcatName, files) => {
    ...
    if (!pdfs.length) {
      setAlertMessage({ severity: "error", message: "Only PDF files allowed" });
      setAlertOpen(true);
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedMainCategory || Object.keys(mappedFiles).length === 0) {
      setAlertMessage({ severity: "error", message: "Category & PDFs required" });
      setAlertOpen(true);
      return;
    }
