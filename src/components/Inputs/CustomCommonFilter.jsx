  ...
  return (
    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, mb: 2 }}>
      <Box sx={{ width: "300px" }}>
        <FormControl fullWidth size="small">
          ...
          <Select
            ...
            onClose={() => setIsOpen(false)}
            displayEmpty
            renderValue={() => isOpen ? "" : <em style={{ color: "#aaa" }}>Select Columns</em>}
            MenuProps={{
              PaperProps: {
                sx: {
                  '& .MuiMenuItem-root': { minHeight: '30px', paddingY: '4px' },
      ...
      {selectedFilters.map((field) => {
        const col = filterableColumns.find((c) => c.field === field);
        return (
          <Box sx={{ width: 300 }} key={field}>
            {/* <CustomTextField
              name={col.field}
              label={col.headerName}
              value={filterValues[col.field] || ""}
              handleChange={(e) => onFieldValueChange(col.field, e.target.value)}
            /> */}
             ...
             {col?.type === "date" ? (
              <CustomDatePicker
                        ...
                        label={col?.headerName}
                        value={filterValues[col.field] || ""}
                        handleChangeAdvance={(name, newValue) => onFieldValueChange(name, newValue)}
                        maxDate={new Date()}
                        required
                      />
            ) : (
