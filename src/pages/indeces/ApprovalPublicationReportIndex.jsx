...
function ApprovalPublicationIndex() {  
  ...
  const onClickPrint = async(rowData)=> {
    ...
    const timeLineData = await getTimeLineData(rowData.row?.emp_id,rowData.row?.incentive_approver_id,rowData);
    let list = {
  ...
  const getTimeLineData = async (empId,incentiveId,rowValue) => {
    try {
      let timeLineLists = [];
  ...
  const getData = async () => {
    try {
      ...
      const res = await axios
        .get(
          `api/employee/publicationBasedOnEmpId`
        );
      if (res.status == 200 || res.status == 201) {
        setLoading(false);
