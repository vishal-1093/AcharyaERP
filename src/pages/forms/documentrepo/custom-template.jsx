...
const CustomTemplate = () => {
    ...
    const savePdf = () => {
        if(valueRef.current === "") return alert("Please provide proper value to save.")
        if(categoryType === "Select Category") return alert("Please select category")
    ...
    return (
        <Grid container sx={{ display: "flex", flexDirection: "column" }} mt={2}>
            <Grid container sx={{ display: "flex", flexDirection: "row", gap: "15px" }}>
                ...
                <Grid item xs={12} sm={12} md={2} lg={2} xl={2}>
                    <Button variant="text" 
                        startIcon={<SyncIcon />}
                        onClick={generatePdf}>Sync</Button>
                </Grid>
                <Grid item xs={12} sm={12} md={2} lg={2} xl={2}>
                    <Button variant="contained"
                        onClick={savePdf}>Save / Lock</Button>
                </Grid>
            </Grid>
            <Grid container sx={{ display: "flex", flexDirection: "row" }} mt={3}>
                <Grid item xs={12} sm={12} md={6}>
                    <Editor placeholder="Write Something" saveData={e => handleTypedData(e)} />
                </Grid>
                <Grid item xs={12} sm={12} md={6} sx={{ position: "relative" }}>
                    {loading && (
                        <CircularProgress
                            sx={{ position: "absolute", width: "100%", top: "42%", left: "50%" }}
                        />
                    )}
                    <object
