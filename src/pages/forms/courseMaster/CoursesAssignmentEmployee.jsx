...
function CoursesAssignmentEmployee() {
  ...
  useEffect(() => {
    ...
    getCourseTypeData();
    if (pathname.toLowerCase() === "/course-assignment-for-employee") {
      setIsNew(true);
      setCrumbs([
        {
          name: "Course Master",
  ...
  const getCourseData = async () => {
    await axios
      .get(`/api/academic/getCourseAssignmentEmployeeBasedOnUserId/${userID}`)
      .then((res) => {
        setCourseOptions(
          res.data.data.map((obj) => ({
            value: obj.course_id,
    ...
    await axios
      ...
      .then((res) => {
        ...
        setCrumbs([
          {
            name: "Course Master",
            link: "/course-assignment-for-employee-index",
          },
          { name: "Course Assignment" },
          { name: "Update" },
        ]);
      ...
      await axios
        ...
        .then((res) => {
          ...
          if (res.status === 200 || res.status === 201) {
            navigate("/course-assignment-for-employee-index", {
              replace: true,
            });
            setAlertMessage({
              severity: "success",
...
export default CoursesAssignmentEmployee;
