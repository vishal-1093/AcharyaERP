...
const FRROBonafied = () => {
    ...
    useEffect(() => {
        const getPdf = async () => {
            setPreviewPath(await GenerateFrroPdf(values, withLetterhead === "Yes" ? true : false))
        }
    ...
    const getStudentList = async () => {
        axios.get("/api/frro/getStudentDetailsListForFrro")
            .then(res => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            setLoading(true)
            const res = await axios.get(`/api/frro/getStudentDetailsForFrro?auid=${auid}`)
    ...
    const updateData = () => {
        ...
        axios.put(`/api/frro/updateFrro?studentId=${data.studentId}`, payload)
            .then(res => {
                setAuid("")
                setNoRecordFound(true)
        ...
        {showPreview && <ModalSection>
            <ModalContainer>
                <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <CloseIcon style={{ fontSize: "30px", cursor: "pointer", position: "absolute", right: "30px" }} onClick={() => setShowPreview(!showPreview)} />
                </Box>
                <object
