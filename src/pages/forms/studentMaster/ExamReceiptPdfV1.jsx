      ...
      >
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 2, mb: 2 }}>
          ...
          <Box sx={{ mt: 3 }}>
              ...
              <Grid item xs={4}>
                <Typography variant="body1"><strong>Student Name: </strong>{receiptData?.feeReceiptWithStudentDetails?.[0]?.student_name}</Typography>
              </Grid>
              <Grid item xs={4}>
                <Typography variant="body1"><strong>Receipt No.:</strong> {receiptData?.feeReceiptWithStudentDetails?.[0]?.fee_receipt}</Typography>
              </Grid>
               <Grid item xs={4}>
                <Typography variant="body1"><strong>Fee Category:</strong> 
                {
             ...
             ?.fee_admission_category_short_name
                 ...
                 <Grid item xs={4}>
                <Typography variant="body1"><strong>AUID:</strong> {receiptData?.feeReceiptWithStudentDetails?.[0]?.auid}</Typography>
              </Grid>
              <Grid item xs={4}>
                <Typography variant="body1"><strong>Receipt Date:</strong> {moment(receiptData?.feeReceiptWithStudentDetails?.[0]?.created_date).format("DD-MM-YYYY")}</Typography>
              </Grid>
              <Grid item xs={4}>
                <Typography variant="body1"><strong>Mobile:</strong> {receiptData?.feeReceiptWithStudentDetails?.[0]?.mobile}</Typography>
              </Grid>
                <Grid item xs={4}>
                <Typography variant="body1"><strong>USN:</strong>{receiptData?.feeReceiptWithStudentDetails?.[0]?.usn}</Typography>
              </Grid>
              <Grid item xs={4}>
                <Typography variant="body1"><strong>Financial Year:</strong> {receiptData?.feeReceiptWithStudentDetails?.[0]?.financial_year}</Typography>
              </Grid>
              <Grid item xs={4}>
                <Typography variant="body1"><strong>Created By:</strong> {receiptData?.feeReceiptWithStudentDetails?.[0]?.created_username}</Typography>
              </Grid>
             </Grid>
          </Box>
          ...
          <thead>
           <tr style={{textAlign: "center"}}>
             <th style={{ border: "1px solid black", padding: "3px 5px", lineHeight: "1.6" }}>Particulars</th>
             {years?.map((year, i) => (  
             <th style={{ border: "1px solid black", padding: "3px 5px", lineHeight: "1.6" }} key={i}>{`Sem-${year}`}</th>
              ))}
             <th style={{ border: "1px solid black", padding: "3px 5px", lineHeight: "1.2" }}>Total</th>
           </tr>
         </thead>
         <tbody>
...
<tr style={{textAlign: 'center'}}>
  ...
  <td style={{ textAlign: "end", padding: "8px", border: "1px solid black" }}>
    <strong>{receiptData?.totalPaidAmount}</strong>
          ...
          <Typography variant="body1"><strong>Transaction No.:</strong> {receiptData?.feeReceiptWithStudentDetails?.[0]
                   ?.transaction_no ?? "NA"}</Typography>
          <Typography variant="body1"><strong>Transaction Date:</strong> {receiptData?.feeReceiptWithStudentDetails?.[0]
                   ?.transaction_date ?? "NA"}</Typography>
          </Box>
          )}
           <Typography variant="body1"><strong>Remarks: </strong> {receiptData?.feeReceiptWithStudentDetails?.[0]?.remarks}</Typography>
