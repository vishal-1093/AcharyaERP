...
function HolidayCalenderIndex() {
...

    {
      ...
      getActions: (params) => [
        <IconButton
          onClick={() =>
            navigate(
              `/HolidayCalenderMaster/HolidayCalender/Update/${params.row.id}`
            )
          }
        >
          <EditIcon />
    ...
    },
    {
      ...
      getActions: (params) => [
        <IconButton
          onClick={() =>
            navigate(
              `/HolidayCalenderMaster/DeAssignDepartment/${params.row.id}`
            )
          }
        >
  ...
  const getData = async () => {
    await axios
      .get(
        `/api/fetchAllHolidayCalenderDetails?page=${0}&page_size=${100}&sort=created_date`
      )
      .then((res) => {
        console.log(res);
  ...
  return (
    <>
      ...
      <Box sx={{ position: "relative", mt: 2 }}>
        <Button
          onClick={() => navigate("/HolidayCalenderMaster/HolidayCalender/New")}
          variant="contained"
          disableElevation
