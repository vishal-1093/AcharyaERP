  ...
  const columns = [
...

    {
      ...
      getActions: (params) => [
        <IconButton
          onClick={() =>
            navigate(`/ExitFormMaster/exitform/Update/${params.row.id}`)
          }
        >
          <EditIcon />
        </IconButton>,
  ...
  const handleActive = async (params) => {
    ...
    const handleToggle = async () => {
      if (params.row.active === true) {
        await axios
          .delete(`/api/employee/emp_leaves/${id}`)
          .then((res) => {
            if (res.status === 200) {
              getData();
            }
        ...
        await axios
          .delete(`/api/employee/activate_emp_leaves/${id}`)
          .then((res) => {
            if (res.status === 200) {
              getData();
            }
  ...
  return (
    <>
      ...
      <Box sx={{ position: "relative", mt: 2 }}>
        <Button
          onClick={() => navigate("/ExitFormMaster/ExitForm/New")}
          variant="contained"
          disableElevation
