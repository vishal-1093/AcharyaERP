...
function AdmCategoryIndex() {
  ...
  const getData = async () => {
    await axios
      .get(
        `/api/student/fetchAllFeeAdmissionCategoryDetail?page=${0}&page_size=${10000}&sort=created_date`
      )
      .then((Response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Link
            to={`/AdmissionMaster/AdmissionCategory/Update/${params.row.id}`}
          >
  ...
  return (
    <Box sx={{ position: "relative", mt: 2 }}>
      ...
      />
      <Button
        onClick={() => navigate("/AdmissionMaster/AdmissionCategory/New")}
        variant="contained"
        disableElevation
