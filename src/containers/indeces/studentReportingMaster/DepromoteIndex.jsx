...
function DepromoteIndex() {
  ...
  const { acYearId, schoolId, programId, yearsemId, currentYearSem, speId } =
    location?.state;
  ...
  useEffect(() => {
    getData();
    setCrumbs([{ name: "Student De-promote", link: "/StudentDepromote" }]);
  }, []);
  ...
  const handleCreate = async () => {
    ...
    rowData.map((val) => {
      temp.push({
        ...
        student_id: val.student_id,
        current_year:
          val.current_sem % 2 !== 0 ? val.current_year - 1 : val.current_year,
        current_sem: val.current_sem - 1,
        reporting_date: val.reporting_date,
    ...
    <>
      <Box component="form" overflow="hidden" p={1}>
        ...
        <ModalPopup
          ...
          title={`You are about to de-promote the selected  students to ${
            currentYearSem === "1"
              ? `${
              ...
              : `${
                  rowData?.[0]?.current_sem === 1
                    ...
                    ? "2nd Sem"
                    : rowData?.[0]?.current_sem - 1 + "th Sem"
                }`
          }`}
